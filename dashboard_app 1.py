...
def buat_template_excel():
    ...
    Header kolom SENGAJA dibuat sama persis dengan yang tampil di tabel
    dashboard (Tanggal & Waktu terpisah, label berbahasa Indonesia),
    supaya pengguna tidak bingung dengan dua penamaan berbeda.
    """
    contoh = pd.DataFrame([{
        "Tanggal": "2026-08-01",
        "Waktu": "08:00",
        ...
        "AGV 02 - Baterai (%)": 80.0,
        "AGV 03 - Baterai (%)": 90.0,
        "AGV 01 - Isi Daya": False,
        "AGV 02 - Isi Daya": False,
        "AGV 03 - Isi Daya": False,
        "Ada AGV Mengisi Daya": False,
        "Output Produksi (unit)": 12,
    }])
    buffer = io.BytesIO()
...
def normalisasi_unggahan(df_in):
    """
    Menormalkan file yang diunggah pengguna ke skema internal (KOLOM_WAJIB),
    menerima DUA kemungkinan format header:

    (a) Format ramah-pengguna - PERSIS seperti template resmi & tampilan
        dashboard: kolom 'Tanggal' + 'Waktu' terpisah, dan header lain
        memakai label Indonesia seperti 'Beban Mesin (kW)'.
    (b) Format internal/lama - kolom 'Timestamp' tunggal + nama kolom
        asli seperti 'Daya_Mesin_kW' (tetap didukung demi kompatibilitas
        dengan file yang pernah diunduh sebelum template ini diperbarui).
    """
    d = df_in.copy()
    if "Tanggal" in d.columns and "Waktu" in d.columns:
        d["Timestamp"] = pd.to_datetime(
            d["Tanggal"].astype(str) + " " + d["Waktu"].astype(str), errors="coerce"
        )
...
if sumber_data == SUMBER_SIMULASI:
    ...
    if berkas is not None:
        ...
        df_baru = normalisasi_unggahan(df_baru)
