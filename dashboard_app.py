...
def buat_pdf_rekomendasi(ringkasan_teks: dict, gambar_png_bytes, catatan_rekomendasi: str):
    ...
    pdf.add_page()
    pdf.set_font("Helvetica", "B", 16)
    pdf.multi_cell(0, 10, "Rekomendasi Optimasi Pengisian Daya AGV")
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(90, 90, 90)
    pdf.multi_cell(0, 6, f"Dibuat otomatis oleh Smart Recommendation Engine - {pd.Timestamp.now():%Y-%m-%d %H:%M}")
    pdf.ln(4)
    ...
    pdf.set_font("Helvetica", "", 11)
    pdf.multi_cell(0, 6, catatan_rekomendasi)
