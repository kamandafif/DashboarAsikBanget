...
def bersihkan_teks_pdf(teks: str) -> str:
    ...
    tipografi "–"/"—" (sering muncul otomatis dari fitur "smart punctuation"
    saat menyalin teks), tanda kutip miring "'"/"'"/"""/""", elipsis "…",
    tanda seru/centang bergaya (⚠️ ✅), atau emoji apa pun (📊 🔌 dsb.).
    Fungsi ini mengganti karakter-karakter umum tersebut dengan padanan
    ASCII yang aman, lalu membuang sisanya, supaya PDF tidak pernah gagal
    render hanya karena satu karakter tak terduga.
    """
...
def buat_laporan_pdf(judul, ringkasan: dict):
    ...
    pdf.multi_cell(0, 10, bersihkan_teks_pdf(judul))
    pdf.set_font("Helvetica", "", 10)
    pdf.set_text_color(90, 90, 90)
    pdf.multi_cell(0, 6, bersihkan_teks_pdf(
        f"Dibuat otomatis oleh Dashboard Energi & Mobilitas Cerdas - {pd.Timestamp.now():%Y-%m-%d %H:%M}"
    ))
    pdf.ln(4)
...
def buat_pdf_rekomendasi(ringkasan_teks: dict, gambar_png_bytes, catatan_rekomendasi: str):
    ...
    pdf.cell(0, 8, "Ringkasan Kondisi Terdeteksi", ln=1)
    pdf.set_font("Helvetica", "", 11)
    ...
    if gambar_png_bytes:
        gambar_path = "_ilustrasi_rekomendasi_tmp.png"
        with open(gambar_path, "wb") as f:
            f.write(gambar_png_bytes)
        pdf.image(gambar_path, w=180)
        os.remove(gambar_path)
        pdf.ln(4)

    pdf.set_font("Helvetica", "B", 12)
    pdf.cell(0, 8, "Rekomendasi", ln=1)
    pdf.set_font("Helvetica", "", 11)
...
# ============================================================
    ...
    if jumlah_kejadian > 0:
        ...
        if pdf_rekomendasi:
            ...
            st.caption("ℹ️ Unduh PDF rekomendasi membutuhkan paket `fpdf2` (`pip install fpdf2`).")
    else:
