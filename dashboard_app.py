...
def buat_pdf_rekomendasi(ringkasan_teks: dict, gambar_png_bytes, catatan_rekomendasi: str):
    ...
    Seluruh proses dibungkus penanganan galat: kegagalan apa pun di sini
    akan mengembalikan None, TIDAK PERNAH melempar error ke pemanggilnya.
    """
    try:
    ...
    try:
        ...
        if gambar_png_bytes:
            # Gambar dibaca langsung dari memori (io.BytesIO), TANPA menulis
            # file sementara ke disk - menghindari risiko tabrakan nama file
            # antar-pengguna yang mengakses dashboard secara bersamaan.
            try:
                pdf.image(io.BytesIO(gambar_png_bytes), w=180)
                pdf.ln(4)
            except Exception:
...
# ============================================================
    ...
    if jumlah_kejadian > 0:
        ...
        try:
            pdf_rekomendasi = buat_pdf_rekomendasi(
                ringkasan_teks={
            ...
            # Jaring pengaman: apa pun yang gagal di dalam pembuatan PDF
            # TIDAK BOLEH membuat seluruh dashboard ikut berhenti (crash).
            # Cukup tombol unduh yang tidak muncul, modul lain tetap jalan.
            pdf_rekomendasi = None
        ...
        if pdf_rekomendasi:
            ...
            st.caption(
                "ℹ️ PDF rekomendasi belum bisa dibuat saat ini (paket `fpdf2` belum terpasang, "
                "atau terjadi kendala teknis lain). Ringkasan rekomendasi di atas tetap berlaku "
                "dan bisa dibaca langsung di layar ini."
            )
    else:
